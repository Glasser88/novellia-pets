...
- **Offset pagination, only on the records page.** The records page shows 10 per page with `?page=N` (skip/take plus a count query, so "Showing 26–50 of 132" is exact) and a page past the end falls back to the last page. Offset pagination can skip or repeat a row if records are added while paging, which does not matter for one owner browsing their own history; if it ever did (a shared or high-write list), I would switch to cursor pagination keyed on `(date, id)`. The dashboard lists are capped at five with "View all" links, and a pet's own records and the pets list are small by the "handful of pets" assumption, so they are not paginated.
